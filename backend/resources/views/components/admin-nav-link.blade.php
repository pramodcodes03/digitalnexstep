@props(['href', 'icon' => 'default', 'active' => false])
...
$icons = [
    ...
    'pages' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z"/>',
...
<a href="{{ $href }}"
   class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 group {{ $active ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'text-gray-400 hover:text-white hover:bg-gray-800' }}">
    <svg class="w-5 h-5 flex-shrink-0 {{ $active ? 'text-white' : 'text-gray-500 group-hover:text-gray-300' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        {!! $svgPath !!}
    </svg>
    <span x-show="!sidebarCollapsed" class="truncate">{{ $slot }}</span>
    @if($active)
        <span x-show="!sidebarCollapsed" class="ml-auto w-1.5 h-1.5 rounded-full bg-white"></span>
    @endif
</a>
