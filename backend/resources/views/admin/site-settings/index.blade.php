...
@section('content')
                ...
                <div class="px-6 py-6 space-y-6">
                    @foreach($settings as $setting)
                        <div class="flex items-start justify-between gap-4">
                            ...
                            {{-- Delete Button --}}
                            <button type="button" class="flex-shrink-0 mt-6 text-red-500 hover:text-red-700 transition" title="Delete setting"
                                    onclick="if(confirm('Are you sure you want to delete the setting \'{{ $setting->key }}\'?')) document.getElementById('delete-setting-{{ $setting->id }}').submit();">
    ...
    {{-- Delete forms (outside the main form to avoid nesting) --}}
    @foreach($groups as $group => $settings)
        @foreach($settings as $setting)
            <form id="delete-setting-{{ $setting->id }}" action="{{ route('admin.site-settings.destroy', $setting) }}" method="POST" class="hidden">
                @csrf
                @method('DELETE')
            </form>
        @endforeach
