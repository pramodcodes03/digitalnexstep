...
@section('title', 'Edit Feature Module')
@section('breadcrumb', 'Edit Feature Module')
@section('content')
    <div class="max-w-5xl mx-auto">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm">
            <div class="px-6 py-5 border-b border-gray-100">
                <h2 class="text-2xl font-extrabold text-gray-900">Edit Feature Module</h2>
            </div>
            ...
            <div class="p-6">
                ...
                <form action="{{ route('admin.feature-modules.update', $featureModule) }}" method="POST" id="featureModuleForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $featureModule->title) }}" required
                        ...
                        <div>
                            ...
                            <input type="text" name="icon" id="icon" value="{{ old('icon', $featureModule->icon) }}"
                                   class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <input type="text" name="gradient" id="gradient" value="{{ old('gradient', $featureModule->gradient) }}"
                                   class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $featureModule->sort_order) }}" min="0"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="3"
                                      class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $featureModule->description) }}</textarea>
                        </div>
                        ...
                        <div class="flex items-end">
                            <div class="flex items-center gap-3">
                                <label for="is_active" class="relative inline-flex items-center cursor-pointer">
                                    ...
                                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $featureModule->is_active) ? 'checked' : '' }}
                                           class="sr-only peer">
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-100">
                        ...
                        <textarea name="sub_features" id="sub_features" rows="10"
                                  class="hidden w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm font-mono focus:border-blue-500 focus:ring-blue-500">{{ old('sub_features', is_array($featureModule->sub_features) ? json_encode($featureModule->sub_features, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) : ($featureModule->sub_features ?? '[]')) }}</textarea>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let jsonMode = false;
    let subFeatures = [];
    let collapsedState = {}; // Track collapsed state: 'sf-0', 'pt-0-1', 'vid-0-1-2'
    ...
    function createField(label, value, onChange, type) {
        ...
        if (type === 'textarea') {
            ...
            // onChange should be { value, options, callback }
    ...
    function renderVideo(video, sfIdx, ptIdx, vidIdx) {
        ...
        const header = createCollapsibleHeader(
            'Video ' + (vidIdx + 1) + (video.title ? ': ' + video.title : ''),
            collapseKey,
            'purple',
            function () {
                subFeatures[sfIdx].points[ptIdx].videos.splice(vidIdx, 1);
            }
        );
        ...
        if (!isCollapsed(collapseKey)) {
            ...
            body.appendChild(createField('Device Type', {
                current: video.deviceType || 'laptop',
                options: ['laptop', 'tablet', 'mobile'],
                callback: function (v) { video.deviceType = v; }
            }, null, 'select'));
            body.appendChild(createField('Aspect Ratio', {
                current: video.aspectRatio || '16:9',
                options: ['16:9', '4:3', '9:16', '1:1'],
                callback: function (v) { video.aspectRatio = v; }
            }, null, 'select'));
    ...
    function renderPoint(point, sfIdx, ptIdx) {
        ...
        const header = createCollapsibleHeader(
            'Point ' + (ptIdx + 1) + (point.title ? ': ' + point.title : ''),
            collapseKey,
            'green',
            function () {
                subFeatures[sfIdx].points.splice(ptIdx, 1);
            }
        );
        card.appendChild(header);
        ...
        if (!isCollapsed(collapseKey)) {
            ...
            fieldsGrid.appendChild(createField('Carousel Layout', {
                current: point.carouselLayout || 'cards',
                options: ['cards', 'filmstrip', 'grid', 'spotlight'],
                callback: function (v) { point.carouselLayout = v; }
            }, null, 'select'));
            ...
            videosHeader.innerHTML = '<h5 class="text-xs font-bold text-purple-700 uppercase tracking-wider flex items-center gap-1.5">' +
                '<span class="w-5 h-5 bg-purple-100 text-purple-600 rounded flex items-center justify-center text-[10px] font-bold">' + point.videos.length + '</span>' +
                'Videos</h5>';
            ...
            addVideoBtn.addEventListener('click', function () {
                point.videos.push({ title: '', url: '', duration: '', deviceType: 'laptop', aspectRatio: '16:9' });
                syncToTextarea();
                renderEditor();
            });
            ...
            point.videos.forEach(function (video, vidIdx) {
                videosContainer.appendChild(renderVideo(video, sfIdx, ptIdx, vidIdx));
            });
    ...
    function renderSubFeature(sf, sfIdx) {
        ...
        const header = createCollapsibleHeader(
            'Sub-Feature ' + (sfIdx + 1) + (sf.title ? ': ' + sf.title : ''),
            collapseKey,
            'blue',
            function () {
                subFeatures.splice(sfIdx, 1);
            }
        );
        card.appendChild(header);
        ...
        if (!isCollapsed(collapseKey)) {
            ...
            pointsHeader.innerHTML = '<h4 class="text-xs font-bold text-green-700 uppercase tracking-wider flex items-center gap-1.5">' +
                '<span class="w-5 h-5 bg-green-100 text-green-600 rounded flex items-center justify-center text-[10px] font-bold">' + sf.points.length + '</span>' +
                'Points</h4>';
            ...
            addPointBtn.addEventListener('click', function () {
                sf.points.push({ title: '', description: '', carouselLayout: 'cards', videos: [] });
                syncToTextarea();
                renderEditor();
            });
            ...
            sf.points.forEach(function (point, ptIdx) {
                pointsContainer.appendChild(renderPoint(point, sfIdx, ptIdx));
            });
    ...
    function renderEditor() {
        ...
        if (subFeatures.length === 0) {
            editor.innerHTML = '<div class="text-center py-8 text-gray-400">' +
                '<p class="text-sm">No sub-features yet. Click "Add Sub-Feature" to get started.</p>' +
                '</div>';
            return;
        }

        subFeatures.forEach(function (sf, sfIdx) {
            editor.appendChild(renderSubFeature(sf, sfIdx));
        });
    }
