...
@section('content')
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm">
        ...
        <div class="flex items-center justify-between px-6 py-5 border-b border-gray-100">
            <h1 class="text-2xl font-extrabold text-gray-900">Hero Slides</h1>
        ...
        <div class="px-6 py-4 border-b border-gray-100">
            <form method="GET" action="{{ route('admin.hero-slides.index') }}" class="flex items-center gap-3">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search hero slides..."
                ...
                @if(request('search'))
                    <a href="{{ route('admin.hero-slides.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Clear</a>
                @endif
            </form>
        </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($items as $item)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 text-gray-600">{{ $item->subtitle }}</td>
                            <td class="px-6 py-4">
                                @if($item->is_active)
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-3">
                                    <a href="{{ route('admin.hero-slides.edit', $item) }}" class="text-blue-600 hover:text-blue-700 text-sm font-medium">Edit</a>
                                    <form action="{{ route('admin.hero-slides.destroy', $item) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this hero slide?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-400">
                                No hero slides found. <a href="{{ route('admin.hero-slides.create') }}" class="text-blue-600 hover:underline">Add your first one</a>.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
