...
@section('content')
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm">
        <div class="px-6 py-5 border-b border-gray-100">
            <h1 class="text-2xl font-extrabold text-gray-900">Edit Testimonial</h1>
        </div>

        <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST" enctype="multipart/form-data" class="px-6 py-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="5" required
                          class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500">{{ old('content', $testimonial->content) }}</textarea>
            </div>

            {{-- Rating --}}
            <div>
                <label for="rating" class="block text-sm font-semibold text-gray-700 mb-1">Rating</label>
                <input type="number" name="rating" id="rating" value="{{ old('rating', $testimonial->rating) }}" min="1" max="5"
                       class="w-full max-w-[200px] rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500">
                <p class="mt-1 text-xs text-gray-500">Rating from 1 to 5</p>
            </div>
            ...
            <div>
                ...
                @if($testimonial->image)
                    <div class="mb-3">
                        <img src="{{ $testimonial->image }}" alt="{{ $testimonial->name }}" class="w-20 h-20 object-cover rounded-xl border border-gray-200">
                    </div>
                @endif
